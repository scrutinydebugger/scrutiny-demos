...
{  
  ...
  ctrl = I_AM_LMS303DLHC; // LSM303DLHC does not have WHO_AM_I register
...
/**
  ...
  * @retval None
  */
void LSM303DLHC_AccReadXYZ(ACCELERO_XYZTypeDef *pData)
{
  int32_t pnRawData[3];
  uint8_t ctrlx[2]={0,0};
  int8_t buffer[6];
  ...
  if(!(ctrlx[0] & LSM303DLHC_BLE_MSB)) 
  {
    for(i=0; i<3; i++)
    {
      pnRawData[i]=((int32_t)((uint32_t)buffer[2*i+1] << 8) + buffer[2*i]);
    }
  }
  else /* Big Endian Mode */
  {
    for(i=0; i<3; i++)
    {
      pnRawData[i]=((int32_t)((uint32_t)buffer[2*i] << 8) + buffer[2*i+1]);
    }
  }
  ...
  pData->x = pnRawData[0] * sensitivity;
  pData->y = pnRawData[1] * sensitivity;
  pData->z = pnRawData[2] * sensitivity;
}
