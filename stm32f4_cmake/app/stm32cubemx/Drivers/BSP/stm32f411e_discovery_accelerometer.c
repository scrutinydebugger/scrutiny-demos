...
uint8_t BSP_ACCELERO_Init(void)
{
  ...
  if(Lsm303dlhcDrv.ReadID() == I_AM_LMS303DLHC)
  {
    ...
    Accelero_InitStructure.AccOutput_DataRate = LSM303DLHC_ODR_400_HZ;
    Accelero_InitStructure.Axes_Enable        = LSM303DLHC_AXES_ENABLE;
    Accelero_InitStructure.AccFull_Scale      = LSM303DLHC_FULLSCALE_4G;
...
/**
  ...
  * @param  pDataXYZ: Pointer to 3 linear acceleration axes.
  *                   pDataXYZ[0] = X axis, pDataXYZ[1] = Y axis, pDataXYZ[2] = Z axis
  */
void BSP_ACCELERO_GetXYZ(ACCELERO_XYZTypeDef *pDataXYZ)
{
  int32_t SwitchXY = 0;
